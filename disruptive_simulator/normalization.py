...
@Description: Disruptive vetorial cellullar automata simulation module.
              Encharged of the normalization of a field or fields. Two 
              functions have been designed. One to perform standard 
              normalization based on a field given, where distance (aptitude)
              is inverted. Second one perform a normalization of a set of
              values that are part of a same variable (attraction-repulsion)
              and so that will be normalized as a single set.
"""
