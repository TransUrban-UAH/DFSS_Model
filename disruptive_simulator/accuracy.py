...
@Date: Tue Feb 16 11:22:22 2021
@Version: 1.4
@Description: Disruptive vetorial cellullar automata simulation module.
              Encharged of the accuracy assesment of the simulation results.
              Given a GDF with a simulated and a "workshopped" for each parcel
              performs an accuracy assesment using diferent metrics: confusion
              matrix for each type of analysis, overall PA and UA, Figure of
              Merit and Growth Simulation Accuracy. Unique structured DataFrame
...
def accuracy_assessment(gdf):
    ...
    # DF default method
    try:
        A = len(gdf.query("DYN_BOOL == 1 and DEVELOP.isnull()").index)
        B = len(gdf.query("DEVELOP == USE_2050").index)
        C = len(gdf.query("DYN_BOOL == 1 and DEVELOP.notnull() and DEVELOP != USE_2050").index)
        D = len(gdf.query("DYN_BOOL == 0 and DEVELOP.notnull()").index)
        E = len(gdf.query("DYN_BOOL == 0 and DEVELOP.isnull()").index)
        
    except:
        #alternative to manage ndarray related error
